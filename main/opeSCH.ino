...
const char* const str_opeSCH[] PROGMEM = {str_opeSCH0, str_opeSCH1,
                                          str_opeSCH2};
...
void opeSCH(void) {
    ...
    if (cposy == 1) {
        ...
        InputDataButtom(cposp, cposx, cposy, K_DIGIT, ptr_crosskey->kpos, mi,
                        mx);
    } else if (cposy == 2) {
        if ((cposx >= 4) && (cposx < 12)) {
            mi = '0';
            mx = '1';
            InputDataButtom(cposp, cposx, cposy, K_DIGIT, ptr_crosskey->kpos,
                            mi, mx);
        }
    }
    ...
    if (ptr_crosskey->kpos & K_ENT) {
        ...
        sprintf(eebuf, "EEPROM %04XH %02X%02X,%02X%02X,%02X%02X,%02X%02X", id,
                sthr, stmn, edhr, edmn, inmn, dumn, rly[0], rly[1]);
...
void getSCHData(int p, int id) {
    ...
    sprintf(lcdbuf, "%02d %02d:%02d %02d:%02d %02d-%02d", id, sthr, stmn, edhr,
            edmn, inmn, dumn);
