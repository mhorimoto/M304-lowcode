    ...
    if (packetSize > 10) {
        ...
        if (xmldecode(&uecsbuf[0])) {
            if (ptr_uecsxmldata->element == ELE_NODESCAN) {
                res_nodescan(UECS_UDP16529.remoteIP(),
                             UECS_UDP16529.remotePort());
                //        debug_uecsxmldata();
            }
        } else {
...
void res_nodescan(IPAddress ripa, unsigned int rport) {
    ...
    sprintf(buf, "%02X%02X%02X%02X%02X%02X", st_m.mac[0], st_m.mac[1],
            st_m.mac[2], st_m.mac[3], st_m.mac[4], st_m.mac[5]);
    clear_uecsbuf();
    cfp = copyFromPROGMEM(&uecsbuf[0],
                          &xmlhead[0]);  // <?xml version=\"1.0\"?><UECS v
    ...
    cfp +=
        copyFromPROGMEM(&uecsbuf[cfp], &res_xmlnode3[0]);  // </VENDER><UECSID>
    ...
    cfp += copyFromPROGMEM(&uecsbuf[cfp],
                           &res_xmlnode6[0]);  // </MAC></NODE></UECS>
