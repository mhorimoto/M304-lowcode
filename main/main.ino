...
const char* const str_main[] PROGMEM = {
    ...
    str_main5,  str_main6,  str_main7,  str_main8, str_main9,
    str_main10, str_main11, str_main12, str_main13};
...
void setup(void) {
    ...
    //  Initialize the CCM table when the initialization switch is in the up
    //  position. Very very dangerous
    j = digitalRead(SW_SAFE);
    if (j == LOW) {
        if ((atmem.read(LC_SEND_START) == 0xff) &&
            (atmem.read(LC_SCH_START) == 0xff)) {  // CCMTABLE
            //        initEEPROM_UECS();
...
void loop(void) {
    ...
    extern void opeSCH(void), opeRTC(void), opeNET(void), opeRUN(int, int, int),
        opeHttpd(EthernetClient);
    extern void UECSupdate16529port(void);
    extern void debugSerialOut(int, int, char*);
    uint8_t InputDataButtom(int, int, int, int, uint8_t, int mi = '0',
                            int mx = '9');
    tmElements_t tm;
...
uint8_t InputDataButtom(int p, int x, int y, int k, uint8_t ud, int mi = '0',
                        int mx = '9') {
    char c;
    extern struct KYBDMEM* ptr_crosskey;
...
void sendUECSpacket(int id, char* v, int lvo) {
    ...
    if (((lvo == 1) && ((lv == 1) || (lv == 2) || (lv == 9)))       // 1sec
        ...
        sprintf(t, xmlDT, ccm_type, room, region, order, priority, v,
                itoaddr(st_m.ip));
...
int copyFromUECSID(char* dest) {
    ...
    char b[13];
    a = LC_UECS_ID;
    sprintf(b, "%02X%02X%02X%02X%02X%02X", atmem.read(a), atmem.read(a + 1),
            atmem.read(a + 2), atmem.read(a + 3), atmem.read(a + 4),
            atmem.read(a + 5));
    for (a = 0; a < 12; a++) {
        *dest = b[a];
        dest++;
...
IPAddress getIPAddressFromEEPROM(int a) {
    IPAddress r;
    r = IPAddress(atmem.read(a), atmem.read(a + 1), atmem.read(a + 2),
                  atmem.read(a + 3));
    return (r);
}
...
void configure_wdt(void) {
    ...
    WDTCSR =
        0b00001000 |
        0b100001;  // clr WDIE: interrupt enabled
