...
// 分に変換する関数
int toMinutes(int hours, int minutes) { return hours * 60 + minutes; }
...
int isOn(int H1, int M1, int H2, int M2, int M0, int D0, int TH, int TM) {
    ...
    int elapsedTime =
        (currentTime - startTime) % cycleTime;  // 開始時刻からの経過時間
...
int isOnSecond(int H1, int Mi1, int S1, int H2, int Mi2, int S2, long S0,
               long D0, int TH, int TMi, int TS) {
    // 開始時刻と終了時刻を秒に変換
    ...
    long elapsedTime =
        (currentTime - startTime) % cycleTime;  // 開始時刻からの経過時間 (秒)
...
void opeRUN(int hr, int mn, int sec) {
    ...
    for (id = 0; id < CCM_TBL_CNT_RX; id++) {
        ...
        if (flb_rx_ccm[id].valid != 0xff) {
            ...
            //            rt[id] =
            //            isOn(flb_rx_ccm[id].sthr,flb_rx_ccm[id].stmn,flb_rx_ccm[id].edhr,flb_rx_ccm[id].edmn,
            //                        flb_rx_ccm[id].dumn,flb_rx_ccm[id].inmn,hr,mn);
            rt[id] = isOnSecond(flb_rx_ccm[id].sthr, flb_rx_ccm[id].stmn,
                                flb_rx_ccm[id].stsc, flb_rx_ccm[id].edhr,
                                flb_rx_ccm[id].edmn, 0, flb_rx_ccm[id].inmn,
                                flb_rx_ccm[id].dumn, hr, mn, sec);
            sprintf(t, "%d:%d=isOnSecond(%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d)\n",
                    id, rt[id], flb_rx_ccm[id].sthr, flb_rx_ccm[id].stmn,
                    flb_rx_ccm[id].stsc, flb_rx_ccm[id].edhr,
                    flb_rx_ccm[id].edmn, 0, flb_rx_ccm[id].inmn,
                    flb_rx_ccm[id].dumn, hr, mn, sec);
            debugUdpOut(t);
            if (rt[id] != prt[id]) {
                sprintf(t,
                        "ID=%02d ST=%02d:%02d:%02d ED=%02d:%02d CUR=%02d:%02d "
                        "IN=%02d DU=%02d RT=%d",
                        id, flb_rx_ccm[id].sthr, flb_rx_ccm[id].stmn,
                        flb_rx_ccm[id].stsc, flb_rx_ccm[id].edhr,
                        flb_rx_ccm[id].edmn, hr, mn, flb_rx_ccm[id].inmn,
                        flb_rx_ccm[id].dumn, rt[id]);
                debugUdpOut(t);
                prt[id] = rt[id];
            }
            comparison_exp(id);
            if (flb_rx_ccm[id].cmpccmid[0] != 0xff) {  // 複合条件がある場合
                ...
                for (i = 0; i < 5; i++) {
                    k = y[id];
                    y[id] =
                        combinationCompare(flb_rx_ccm[id].cmbcmp[i],
                                           flb_rx_ccm[id].match_result[i], k);
                    sprintf(t, "ID=%02d-%d:%d=combinationCompare(%d,%d,%d)", id,
                            i, y[id], flb_rx_ccm[id].cmbcmp[i],
                            flb_rx_ccm[id].match_result[i], k);
                    debugUdpOut(t);
                }
            } else {  // 複合条件がない場合
...
void comparison_exp(int id) {
    ...
    if (flb_rx_ccm[id].valid != 0xff) {
        for (i = 0; i < 5; i++) {  // 環境条件5個をチェックする
            y = flb_rx_ccm[id]
                    .cmpccmid[i];  // 比較するuecsM304cmpopeの位置を決定
            if ((i == 0) && (y == 0xff)) {
            ...
            if (y < 0xff) {  // 比較するCCMTYPEがある場合
                if (flb_cmpope[y].remain > 0) {
                    ...
                    switch (flb_rx_ccm[id].cmpope[i]) {
                        case R_EQ:  // ==
                            flb_rx_ccm[id].match_result[i] =
                                (flb_cmpope[y].fval == rval) ? 1 : 0;
                            break;
                        case R_GT:  // >
                            flb_rx_ccm[id].match_result[i] =
                                (flb_cmpope[y].fval > rval) ? 1 : 0;
